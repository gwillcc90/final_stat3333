...
We will use the columns: state, year, abortionratelt15,	abortionrate1517,	abortionrate1819,	abortionrate2024,	abortionrate2529,	abortionrate3034,	abortionrate3539,	abortionrate40plus, which is the rate of abortion per 1000 [young] women in the age range. For example `abortionratelt15` is the rate of abortions of the given U.S. state and year of girls less than 15 years old, and `abortionrate2024` is the rate of abortions of ladies of ages 20-24, per 1000 people.
...
# Is there a statistically significant difference in mean abortion rates among the years.
year <- as.factor(abortions$year)
observed <- anova(lm(MeanRate ~ year, data = abortions))$F[1]
...
for (i in 1:N)
{
 ...
 # Ask about this
 results[i] <- anova(lm(MeanRate.perm ~ year, data = abortions))$F[1]
}

(sum(results >= observed) + 1) / (N + 1)
```

P-value is very small, so we confirm that there is a statistically significant difference in mean abortion rate among the years 
...
```{r}
abortions %>%  select(state,MeanRate) %>%group_by(state) %>% mutate('state_mean' = mean(MeanRate)) %>% arrange(desc(state_mean))
```
