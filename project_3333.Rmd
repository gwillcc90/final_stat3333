...
So which state has the highest abortion rate? It's none other than the "Empire State" - NY has highest rate of abortion with mean = 31.43.
...
qqline(Tstar)
```
From the bootstrap distribution, we can see (28.66640 34.24376) is one of the infinitely many 90% CIs that could contain the true mean abortion rate for New York.
...
In conclusion, we found that there is statically significant evidence in the mean abortion rate from the years 1988 to 2017. 

Using the bootstrap percentile technique, we found that there was a statistically significant drop in abortion rate in 2017 compared to 1988. But upon performing a runs test to see if the data was collected randomly, we saw that the observations were dependent, and needed different tests. We then used the matched pairs, bootstrap resampling technique to find a more narrow interval estimate for the possible difference in population means. We also used a permutation test to confirm that there is a statistically significant difference in abortion rates in 1988 and 2017.
